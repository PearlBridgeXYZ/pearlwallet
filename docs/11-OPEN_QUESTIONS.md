...
- **Owner:** G to ping Pearl chain leads. Bernard can draft the SLIP-44 PR if needed.
...
- **Default if unanswered:** check PearlBridge testnet docs at `(PearlBridge testnet docs)`.
...
- **Default:** pull from `(PearlBridge repo)deployment*.json` once mainnet is finalized.
- **Owner:** Bernard to source from PearlBridge deployment files; Lead to verify on-chain.
...
- **Owner:** Lead engineer + Bernard to PearlBridge team.
...
- **Question:** logo, palette, typography — using one of the existing local tooling palettes (PNW Alpine? Winter Clinical?) or commissioning fresh?
- **Default:** start with `palettes/show.py` to pick from existing palettes; commission a logo from a freelance designer.
- **Owner:** Designer + G.

### Q13. Legal disclaimer / terms of use
- **Question:** do we need lawyer-drafted ToS for a non-custodial wallet? Jurisdiction?
- **Default:** minimal ToS modeled on MetaMask's (open-sourced). G's lawyer to review pre-launch.
- **Owner:** G.
...
- **Owner:** G to choose; Bernard to execute.
...
- **Owner:** Bernard to check; G to engage exchanges/listings.
...
- **Default:** en-only at v1; add fr/es/zh post-launch based on user origin data (which we won't have — so based on G's market priorities).
- **Owner:** G.
